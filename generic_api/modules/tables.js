...
router.get('/ordersall', (req, res) => {
    query('SELECT orders.id,`user_id`,`total`,`status`,orders.created_at,`updated_at`,GROUP_CONCAT(pizzas.name, " x ", order_items.quantity) as name, order_items.quantity FROM `orders` INNER JOIN order_items ON orders.id = order_items.order_id INNER JOIN pizzas ON order_items.pizza_id = pizzas.id GROUP BY orders.id', [], (error, results) => {
...
router.get('/ordersall/:id', (req, res) => {
    ...
    query('SELECT orders.id,`user_id`,`total`,`status`,orders.created_at,`updated_at`,GROUP_CONCAT(pizzas.name, " x ", order_items.quantity) as name, order_items.quantity FROM `orders` INNER JOIN order_items ON orders.id = order_items.order_id INNER JOIN pizzas ON order_items.pizza_id = pizzas.id WHERE user_id = ? GROUP BY orders.id', [userId], (error, results) => {
        if (error) return res.status(500).json({ errno: error.errno, msg: 'Hiba történt az adatbázis lekérdezése közben.', error: error.message });
        res.send(results);
    }, req)
})
